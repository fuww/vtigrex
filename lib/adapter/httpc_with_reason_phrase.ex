defmodule Vtigrex.Adapter.HttpcWithReasonPhrase do
  @moduledoc """
  Tesla Adapter for [httpc](http://erlang.org/doc/man/httpc.html) with adding
  Reason-Phrase after the status code in the X-HTTP-Reason-Phrase header.

  Tesla does not support reason phrase and it is necessary for Vtiger REST API,
  to be able to get the error messages.

  see: https://github.com/elixir-tesla/tesla/blob/master/lib/tesla/adapter/httpc.ex

  see: https://www.vtiger.com/docs/rest-api-for-vtiger#/Response
  ...
  **NOTE** Tesla overrides default autoredirect value with false to ensure
  ...
  defp format_response(env, {_, status, reason_phrase}, headers, body) do
    %{
      env
      | status: status,
        headers: headers |> format_headers() |> add_reason_phrase(reason_phrase),
        body: format_body(body)
    }
  end

  defp add_reason_phrase(headers, reason_phrase) do
    headers ++ [{"x-http-reason-phrase", to_string(reason_phrase)}]
  end
